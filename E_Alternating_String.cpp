...
bool isAlternate(string s){
 ...
 for (ll i = 0; i < s.size(); i++)
 {
    if(i&1) {odd.push_back(s[i]);mpod[s[i]]++;}
    else {even.push_back(s[i]);mpev[s[i]]++;}
 }
for (auto it : mpev)
{
    mxev=max(mxev,it.second);
    mnev=min(mnev,it.second);
}
for (auto it : mpod)
{
    mxod=max(mxod,it.second);
    mnod=min(mnod,it.second);
}
 ...
 for (auto it : even)
 {
    ...
    cout<<ans+((odd.size()-mxod)+(even.size()-mxev))<<endl;
}
 }
 return 0;
 }